<?php 
/*
Plugin Name: Posts 2 Posts API
Plugin URI: https://github.com/jhotadhari/posts-2-posts-api
Description: REST API extension for Posts 2 Posts plugin
Version: 0.0.1
Author: jhotadhari
Author URI: https://waterproof-webdesign.info/
License: GNU General Public License v2 or later
License URI: http://www.gnu.org/licenses/gpl-2.0.html
Text Domain: posts-2-posts-api
...
class P2papi_Posts_2_posts_api {

	protected static $instance = null;
	const VERSION = '0.0.1';
	const DB_VERSION = 0;			// int	increase the number if the database needs an update
	const PLUGIN_SLUG = 'posts-2-posts-api';
	const PLUGIN_NAME = 'Posts 2 Posts API';
	const PLUGIN_PREFIX = 'p2papi';
	protected $deactivate_notice = '';
	protected $deps = array(
		'plugins' => array(
			'posts-to-posts' => array(
				'name'				=> 'Posts 2 Posts',				// full name
				'link'				=> 'https://github.com/scribu/wp-posts-to-posts',	// link
				'ver_at_least'		=> '1.6.5',						// min version of required plugin
				'ver_tested_up_to'	=> '1.6.5',						// tested with required plugin up to
				'function'			=> '_p2p_init',					// test by function
			),
		),
	...
	public function start_plugin() {
		if ( $this->check_dependencies() ){
			...
			do_action('p2papi_plugin_loaded');
		} else {
	...
	public function on_activate() {
		if ( $this->check_dependencies() ){
			...
			do_action('p2papi_on_activate_before_flush');
			flush_rewrite_rules();
			$this->maybe_update();
			do_action('p2papi_plugin_activated');
		} else {
			add_action( 'admin_init', array( $this, 'deactivate' ) );
			wp_die(
	...
	public function on_deactivate() {
		$this->add_roles_and_capabilities();
		do_action('p2papi_on_deactivate_before_flush');
		flush_rewrite_rules();
		do_action('p2papi_plugin_deactivated');
	}

	public static function on_uninstall() {
		do_action('p2papi_plugin_uninstalled');
	}
	...
	protected function init_options() {
		update_option( 'p2papi_version', self::VERSION );
		add_option( 'p2papi_db_version', self::DB_VERSION );
	}
	...
	protected function maybe_update() {
		if ( get_option( 'p2papi_db_version' ) < self::DB_VERSION ) {
			// require_once( self::plugin_dir_path() . 'inc/dep/class-p2papi-update.php' );
			// new P2papi_Update();
			// class P2papi_Update()  is missing ??? !!!
		}
	}
...
function p2papi_posts_2_posts_api_init() {
	return P2papi_Posts_2_posts_api::get_instance();
}
p2papi_posts_2_posts_api_init();
